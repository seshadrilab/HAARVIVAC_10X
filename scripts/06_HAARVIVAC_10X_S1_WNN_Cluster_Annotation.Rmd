...
s1_signif_tfs <- s1_signif_markers %>%
...
s1_signif_cytokines <- s1_signif_markers %>%
  dplyr::filter(gene %in% cytokine_list$Symbol) 

s1_cytokines <- s1_markers %>%
  dplyr::filter(gene %in% cytokine_list$Symbol)
...
if(save_output) {
  ...
  saveRDS(s1_tfs, here::here("out/20220331_S1_DE_TFs.rds"))
  saveRDS(s1_cytokines, here::here("out/20220331_S1_DE_cytokines.rds"))
}
