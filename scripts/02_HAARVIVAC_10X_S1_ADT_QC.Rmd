---
title: "HAARVIVAC 10X S1 ADT QC and Processing"
author: "Jolie Phan"
output: 
  html_document:
    toc: yes
...
```{r}
s1 <- readRDS(file = here::here("processed_data/20211118_s1_mdm_singlets.rds"))
save_output <- TRUE
```
...
```{r}
igg1_dat <- GetAssayData(s1, slot = "data", assay = "ADT") %>%
  as.data.frame() %>%
...
igg2a_dat <- GetAssayData(s1, slot = "data", assay = "ADT") %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Marker") %>%
...
igg2b_dat <- GetAssayData(s1, slot = "data", assay = "ADT") %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Marker") %>%
...
iso_control_dat <- GetAssayData(s1, slot = "data", assay = "ADT") %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Marker") %>%
...
**Spike 1** 99th percentile expression level for each IgG isotype control:
...
```{r}
adt_assay <- GetAssay(s1, assay = "ADT")
Key(object = adt_assay) <- "adtbc_"
s1[["ADT_bc"]] <- adt_assay
s1[["ADT_bc"]] <- SetAssayData(s1[["ADT_bc"]], slot = "data", new.data = bc_adt_dat)
```
...
s1 <- ScaleData(s1) 
s1 <- RunPCA(s1, reduction.name = "apca", 
                         reduction.key = "APC_")
ElbowPlot(s1)
...
adt_data <- GetAssayData(s1, slot = "data", assay = "ADT_bc")
adt_dist <- dist(t(adt_data))
s1[["umap_adt"]] <- RunUMAP(adt_dist, assay = "ADT_bc", reduction.key = "adtUMAP_")
s1[["adt_snn"]] <- FindNeighbors(adt_dist)$snn
s1 <- FindClusters(s1, resolution = 0.4, 
                               graph.name = "adt_snn")

DimPlot(s1, reduction = "umap_adt", label = TRUE, pt.size = 0.7) +
  ggtitle("Spike 1") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
Visualize surface protein expression and determine what the broad cell types are.
...
if(save_output) {
  cairo_pdf(file=here::here("out/20220323_S1_ADT_Violin_Plots.pdf"), width=12, height=10, onefile = TRUE)
...
Based on the ADT expression levels, these were the broad cell types assigned to each cluster.

Clusters 0, 1, 4, 5, 9, 10: CD4 T cells

Clusters 3, 6, and 8: CD8 T cells
...
i1 <- grepl("0|1|4|5|9|10", s1@meta.data$adt_snn_res.0.4)
i2 <- grepl("3|6|8", s1@meta.data$adt_snn_res.0.4)
i3 <- grepl("2", s1@meta.data$adt_snn_res.0.4) 
i4 <- grepl("7", s1@meta.data$adt_snn_res.0.4) 
i5 <- grepl("11", s1@meta.data$adt_snn_res.0.4) 
s1@meta.data$adt_clusters <- NULL
s1@meta.data$adt_clusters[i1] <- "CD4 T cells"
s1@meta.data$adt_clusters[i2] <- "CD8 T cells"
s1@meta.data$adt_clusters[i3] <- "NK cells"
s1@meta.data$adt_clusters[i4] <- "DCs"
s1@meta.data$adt_clusters[i5] <- "B cells"
...
```{r, fig.width=6, fig.height=4}
Idents(s1) <- "adt_clusters"
p <- DimPlot(s1, reduction = "umap_adt", label = FALSE, pt.size = 0.7, cols = s1_adt_colors) +
  ggtitle("Spike 1") +
  theme(plot.title = element_text(hjust = 0.5))
...
DoHeatmap(s1, features = adt_markers, label = FALSE, assay ="ADT_bc", size = 4) +
  ggtitle("Spike 1") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
Idents(s1) <- "adt_clusters"
levels(s1) <- c("B cells", "DCs", "NK cells", "CD8 T cells", "CD4 T cells")
...
s1_dotplot_data <- s1_dotplot$data

# Get count and proportion of each cell type
clust_count <- s1@meta.data %>%
  count(adt_clusters) %>%
  mutate(Freq = (n/sum(n))*100)

clust_count$adt_clusters <- factor(clust_count$adt_clusters, levels = c("CD4 T cells", "CD8 T cells", "NK cells", "DCs", "B cells"))

# Add cell count to each cluster name
s1_dotplot_data <- s1_dotplot_data %>%
  mutate(clust = case_when(id == "CD4 T cells" ~ "CD4 T cells (3617)",
                           id == "CD8 T cells" ~ "CD8 T cells (657)",
                           id == "NK cells" ~ "NK cells (673)",
                           id == "DCs" ~ "DCs (83)",
                           id == "B cells" ~ "B cells (23)"))

s1_dotplot_data$clust <- factor(s1_dotplot_data$clust, levels = c("CD4 T cells (3617)", "CD8 T cells (657)", "NK cells (673)", "DCs (83)", "B cells (23)"))
...
# Extract legend
s1_dotplot_legend <- as_ggplot(get_legend(s1_fancy_dotplot +
...
table(s1$MULTI_ID_global)

s1_sub <- subset(s1, subset = adt_clusters %in%
                               c("CD4 T cells", "CD8 T cells"))

# How many cells do we have after filtering out non-T cells?
table(s1_sub$MULTI_ID_global)
```
...
if(save_output) {
  saveRDS(s1, file = here::here("processed_data/20220323_s1_with_adt_labels.rds"))
  saveRDS(s1_sub, file = here::here("processed_data/20220323_s1_tcells_only.rds"))
  
  cairo_pdf(file=here::here("out/S1_ADT_dotplot.pdf"), width=3.5, height=2,
        onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  print(s1_fancy_dotplot)
  dev.off()
  
  cairo_pdf(file=here::here("out/S1_ADT_dotplot_legend.pdf"), width=4, height=2,
        onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  print(s1_dotplot_legend)
  dev.off()
}
```
